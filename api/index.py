...
assets/app-part3.js), audit, plans (cycle de planification versionné
draft → reviewed → validated | rejected) et anesthesie (dossier
pré-anesthésique + suivi USI). Ces deux derniers sont purement data (SQLAlchemy
sur le Postgres externe) : c'est le cœur du dossier périopératoire, il doit
rester disponible même quand la segmentation/DICOM ne l'est pas.
...
logger = logging.getLogger("generalsurg.vercel")
...
app = FastAPI(title="GeneralSurg Plan MIMO — API légère (Vercel)", version="2.1.0-vercel", lifespan=lifespan)
